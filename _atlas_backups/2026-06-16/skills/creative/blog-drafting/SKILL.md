...
# Pre-flight: check for post_url tags in remaining scheduled posts
if grep -rn "post_url" .scheduled/ 2>/dev/null; then
  echo "ERROR: post_url tags found in .scheduled/ — fix before publishing"
  exit 1
fi
...
When the full pipeline isn't available (no Python tools venv), use cairosvg + Pillow directly:

```bash
# Create a temp venv if one doesn't exist
uv venv /tmp/svg_venv
source /tmp/svg_venv/bin/activate
uv pip install cairosvg pillow

# Convert a single SVG to 1200×630 WebP
/tmp/svg_venv/bin/python3 << 'PYEOF'
import cairosvg
from PIL import Image
import io, os

svg_path = "assets/blog/cover-slug.svg"
webp_path = "assets/img/cover-slug.webp"

png_data = cairosvg.svg2png(url=svg_path, output_width=1200, output_height=630)
img = Image.open(io.BytesIO(png_data)).convert("RGB")
img.save(webp_path, "WEBP", quality=85)
print(f"Saved {webp_path} ({os.path.getsize(webp_path)/1024:.0f} KB)")
PYEOF
```
