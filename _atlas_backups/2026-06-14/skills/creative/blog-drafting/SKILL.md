...
#    Use positional '1s/^pick/edit/' to target the first commit in the
#    rebase list (the offending one). Avoids fragile SHA matching.
...
#    Verify nobody else pushed to the branch first
...
4. Commit all `.scheduled/` + `assets/blog/cover-*.svg` in one batch commit
5. The existing daily cron picks them up automatically starting with the earliest date

**IMPORTANT — cleanup after publish:** Because `.scheduled/` files are tracked,
`mv` creates an unstaged deletion. After each publish, commit the removal:

```bash
# After cron pushes successfully, clean up the tracked .scheduled/ deletion:
git add -A
git commit -m "Clean up staged scheduled file for YYYY-MM-DD"
git push origin main
```
...
- ✅ Multimodal Security Attacks (`2026-06-11-multimodal-attacks`)

**Staged for daily publish (cron at 14:05 EAT):**
- Jun 12: ML Secrets Management (`2026-06-12-ml-secrets-management`) — **PUBLISHED**

### 5. Building AI Agents from Scratch (STAGED — 7 posts, Jun 15–21)
- Jun 15: Agent Fundamentals: ReAct, Tools & Reasoning Loops (`agent-fundamentals`)
- Jun 16: Memory Systems: Short-Term, Long-Term & Persistent (`agent-memory-systems`)
- Jun 17: Tool Use & Function Calling (`agent-tool-calling`)
- Jun 18: Multi-Agent Systems with LangGraph & CrewAI (`multi-agent-systems`)
- Jun 19: Agent Observability & Debugging (`agent-observability`)
- Jun 20: Deploying Agents to Production (`agent-production-deployment`)
- Jun 21: Building Secure AI Agents (`agent-security`)

### 6. African AI / Real-World ML (STAGED — 7 posts, Jun 22–28)
- Jun 22: State of AI in Africa (`african-ai-landscape`)
- Jun 23: Swahili NLP (`swahili-nlp`)
- Jun 24: Low-Resource NLP (`low-resource-nlp`)
- Jun 25: Mobile-First AI with TensorFlow Lite (`mobile-first-ai`)
- Jun 26: African AI Communities (`african-ai-communities`)
- Jun 27: AI for Agriculture in Kenya (`ai-for-agriculture`)
- Jun 28: Constrained Environment AI (`constrained-environment-ai`)

### 7. AI Infrastructure & Serving (STAGED — 6 posts, Jun 29–Jul 4)
- Jun 29: Model Serving 101 (`model-serving-101`)
- Jun 30: vLLM & High-Throughput LLM Serving (`vllm-llm-serving`)
- Jul 1: GPU Optimization for ML (`gpu-optimization`)
- Jul 2: Kubernetes for ML with Kserve (`kubernetes-ml`)
- Jul 3: ML CI/CD Automation (`ml-cicd`)
- Jul 4: Monitoring ML Systems in Production (`ml-monitoring`)
