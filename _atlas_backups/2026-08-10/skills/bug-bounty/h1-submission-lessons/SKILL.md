...
## Pipeline (as of Aug 2026 — verified, not yet submitted)
| # | Finding | Program | Status | Proof |
|---|---------|---------|--------|-------|
| 7 | Supabase self-hosted Studio admin API auth bypass (CVSS 9.8-10) | HackerOne (supabase) | READY TO SUBMIT — live-verified on docker | `/home/pro-g/Dev/REPORTS/Supabase/002-live-verification/REPORT.md` + poc/verify_all.sh + forge_jwt.py |
| 8 | blog.etsy.com XML-RPC fully enabled (system.multicall, pingback.ping) | Bugcrowd (Etsy) | P4 candidate — verified live | `/home/pro-g/Dev/REPORTS/Etsy/002-agent2/REPORT.md` |
| 9 | Skoda swagger/actuator → Azure AD SSO tenant+client leak | Intigriti (Skoda) | Low/info — verified live | `/home-pro-g/Dev/REPORTS/Skoda/002-agent3/REPORT.md` |
| 10 | Skoda VW ID user-enumeration oracle (login/identifier redirect diff) | Intigriti (Skoda) | Low — reproducible 2/2 | same report |
| — | Fireblocks BAM oracle + CRT extraction | HackerOne (fireblocks_mpc) | **WITHDRAWN** — inherited PoC fails to reproduce | `007-agent5/REPORT.md` |
| — | Rapyd ngrok webhook bug | Bugcrowd (Rapyd) | OOS — merchant-side sample code | GitHub issue venue only |

## Lesson Bank Integration (MANDATORY)

**Every submission outcome MUST be recorded in the lesson bank** at `~/Dev/ATLAS-LEARNINGS/LESSONS.md` (§01 Bug Bounty) — not just kept in this skill's table. This is the durable store; this skill is the playbook. After ANY submission (accepted, duplicate, informative, N/A, OOS):
1. Add a row to the Track Record table above
2. Append a dated lesson entry to `~/Dev/ATLAS-LEARNINGS/LESSONS.md` §01
3. If memory is near full, compress MEMORY.md to a pointer (`See LESSONS.md §01`)
4. Patch this skill with any NEW pattern discovered (duplicate cause, rejection reason, program stance)

Use the `atlas-lesson-bank` skill (skill_view) for the full workflow.
