...
# ============================================================================
# STEP 8.5: Sync ATLAS lesson bank (durable memory — survives machine loss)
# ============================================================================
LESSONS_SRC=~/Dev/ATLAS-LEARNINGS
if [ -d "$LESSONS_SRC" ]; then
  mkdir -p "$ATLAS_DIR/lessons" "$BACKUP_DIR/lessons"
  rsync -a --update "$LESSONS_SRC/" "$ATLAS_DIR/lessons/" 2>/dev/null
  rsync -a --update "$LESSONS_SRC/" "$BACKUP_DIR/lessons/" 2>/dev/null
  echo "ATLAS lesson bank synced to repo + backup"
else
  echo "WARNING: $LESSONS_SRC not found — lesson bank not synced"
fi
...
for dir in skills memory config scripts jobs lessons; do
...
if scan_for_secrets "$ATLAS_DIR" 2>/dev/null; then
  git add _hermes_backups/ skills/ memory.md lessons/ 2>/dev/null
  ...
  echo "  Files reverted."
  git checkout -- _hermes_backups/ skills/ memory.md lessons/ 2>/dev/null || true
fi
