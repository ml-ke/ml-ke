...
### 7.8 Ruling out the environment before blaming the app

When a service or test harness fails to start, separate "the app is broken" from "this
environment will not let it run" **before** reporting failures:

- A near-universal tell: the process logs `Listening on http://…` and then
  `createTcpServer: permission denied` / `Failed to create server`. Every dependency
  loaded and the app initialized; only the socket bind was refused. Report the
  constraint, not N app failures.
- Read the failing process's own log for its LAST few lines before its exit — package
  load output followed by a bind error is a completely different diagnosis from an
  early `there is no package called X`.
- A retry loop that waits for a port that can never open produces a wall of false
  failures. Check the first failure's log before letting the loop run to completion;
  aborting early saves the whole run and prevents publishing junk results.
...
- **`sudo -A` breaks when SUDO_PASSWORD is in the agent env**: the terminal wrapper auto-injects `-S`, so askpass dies with `sudo: the -A and -S options may not be used together`. Run privileged commands as `env -u SUDO_PASSWORD sudo -A …` (and put `echo` in the askpass script — a bare value line makes the shell try to execute the password as a command). Verified 2026-09-14.
- **A leftover `/etc/sudoers.d/*.bak` with `NOPASSWD: ALL` is INERT, not live** — sudo refuses to read filenames containing a dot. Never assume a stray rule is active: check `sudo -l -U <user>` for the effective list. It is still a landmine (one `mv` restores passwordless root) and a classic post-exploitation target — flag it and make future sudoers backups live outside `/etc/sudoers.d/`.
- **Numeric uid collisions across namespaces**: `find / -uid 1001` on this host returns dozens of files inside `/var/lib/docker/.../containerd/…/snapshots/` that belong to a *container's* `openshell` user, not the host account sharing uid 1001. Cross-check with `ps -eo user,uid` / ownership of `/home/<user>` before calling it evidence of access.
- **Dating an account-creation event without logs**: `/etc/passwd`, `/etc/group`, `/etc/shadow` and the matching `/etc/sudoers.d/<user>` all carry the same mtime to the second for a single `useradd`+`visudo` action — `stat -c '%n %y'` on all four proves it was one atomic event. `lastlog -u <user>` → "**Never logged in**" plus no process at that uid = dormant backdoor rather than active intrusion. Rotated `auth.log*` may not reach back far enough to show the `useradd` command line — say so instead of implying attribution.
...
- **Never `pkill -f <pattern>` when the target's name also appears in your own command line** — pkill matches the invoking shell's cmdline too and SIGTERMs your own session mid-command. The symptom is an exit code of `-15` / "terminated by signal 15" with the rest of the command silently never run, which reads like a timeout or a crash. Two traps stack here: (a) the pattern matches the shell that contains the pattern, so use the bracket trick (`pkill -f "10_fin[a-z]lize"`) — and (b) **any other mention of the literal target in the same command re-arms the self-match**, e.g. a trailing `sed -i ... 10_finalize.R` alongside the kill. Kill by PID, or put the kill in its own call with nothing else in it.
- **`sudo -S` fed from `printf` swallows the wrapped command's stdin** — `printf '%s\n' "$PW" | sudo -S cmd` replaces cmd's stdin with the password pipe, so `curl … | sudo tee /etc/…` writes a zero-byte file and the pipeline hangs rather than failing loudly. Diagnose a mysteriously empty config file under `/etc` by checking whether the writer was fed through `sudo -S`. Use `SUDO_ASKPASS=… sudo -A` instead so stdin passes through, and have any `sudo` shim/alias call the absolute `/usr/bin/sudo` or it recurses into itself (`shell level (1000) too high`).
