...
### Terminal background jobs + notify

`terminal(background=True, notify=True)` fires exactly one notification when
the command exits. Put the LONG-RUNNING COMMAND ITSELF in the background call —
never wrap it as `nohup python3 worker.py &` or `cmd &`.

Why: the wrapper shell exits the instant the `&` returns, so the notification
fires immediately (often 20s in, with a "completed normally" message) while the
real worker keeps running orphaned — and when that worker later finishes or
dies, nothing notifies anyone. Symptom: a "Background process completed"
message for a job you know takes 90 minutes, and a log that keeps growing
afterwards (check with `pgrep -af <script>`; the truth is in the log file, not
the notification).

Rule: `terminal(command="python3 worker.py", background=True, notify=True)` —
and have the worker itself append progress to a log file you can tail on the
next turn, since CLI sessions get no live delivery.
