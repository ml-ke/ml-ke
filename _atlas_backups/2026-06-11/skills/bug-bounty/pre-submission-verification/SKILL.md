...
**Our find (Atlassian MCP, 2026)**: The MCP server at mcp.atlassian.com accepted ANY non-empty Bearer token including the literal string "null". Even Basic auth with `dGVzdDp0ZXN0` passed. The server checked only for the existence of an `Authorization` header with non-empty value. See recon-to-exploitation's `references/atlassian-mcp-server-session.md`. 
