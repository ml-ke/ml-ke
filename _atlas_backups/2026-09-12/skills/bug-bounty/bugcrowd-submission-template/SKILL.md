...
**⚠️ CRITICAL — Anti-AI Detection**: As of 2026, Bugcrowd actively flags reports that look AI-generated. Do NOT use this template as a rigid structure. The format below is a guide for your notes — the FINAL report must be narrative, not templated.

**The rule**: Write the PoC first. Run it. Capture the output. Then write 2-3 narrative paragraphs. No section headers. No numbered steps unless the finding genuinely requires them. Paste the curl command inline and show what it returned. Then explain why it matters in one sentence.

**Before (templated — gets flagged):**
```
### Summary
The /api/v2/users endpoint lacks proper authentication...

### Vulnerability Details
**Target:** www.etsy.com
**VRT Category:** BAC > IDOR
**Technical Severity:** P2

### Description
The endpoint does not validate authentication tokens...

### Proof of Concept
1. Send GET to /api/v2/users
2. Observe response contains user data

### Impact
An attacker could enumerate users...
```

**After (narrative — accepted):**
```
GET /api/v2/users returns every user's email and name with no auth at all. Single curl:

curl -s https://www.etsy.com/api/v2/users | json_pp

~14,000 records in one call. No session cookie. No Authorization header. Anybody on the internet gets the full user list.

I confirmed this from three different IPs across two days to rule out caching. Same result every time.
```

**Report Structure Template (for your notes only — convert to narrative before submitting):**
