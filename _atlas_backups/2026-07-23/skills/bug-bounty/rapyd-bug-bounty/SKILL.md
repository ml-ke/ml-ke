...
## Practical Testing Notes (Jun 2026)

### SAML Endpoints — Behind React SPA

All SAML endpoints at `dashboard.rapyd.net` return React SPA HTML (login page), not actual SAML XML:
- `/saml/metadata` returns HTML login page, not IdP metadata XML
- `/saml/login`, `/saml/acs` all return the same React SPA
- Requires an authenticated session with SAML IdP configured to test the 8 attack types

The SAML test IdP mentioned in the promotion is built into the dashboard and only accessible after authentication.
