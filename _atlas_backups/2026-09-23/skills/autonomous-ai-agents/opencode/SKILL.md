...
- OpenCode installed: `npm i -g opencode-ai@1.18.30` (pin the version — a floating `@latest` in a skill that then runs an autonomous coding agent is an unpinned supply-chain path; `brew install anomalyco/tap/opencode` as the alternative)
...
## Provider-Config Pitfalls (deepseek + MCP tool schemas — fixed Sep 2026)

**1. An unknown model id fails with an opaque error.** `"model": "deepseek/deepseek-chat"` (not in OpenCode's registry for that provider) produced only `UnknownError … "Unexpected server error" ref=err_xxxx` — the ref never appears in the log. Confirm valid ids first: `opencode models | grep deepseek`. For the real DeepSeek API (paid key) the working ids are `deepseek/deepseek-v4-pro`, `deepseek/deepseek-flash` — and the API's own `/models` endpoint is the ground truth.

**2. An MCP tool schema can kill EVERY session on a strict provider.** DeepSeek validates tool schemas with a strict regex parser: the Sanity MCP's `dataset_assets_upload_from_url` / `media_assets_upload_from_url` declare `filename.pattern = "^[^/\\\0]+$"`, which DeepSeek rejects with HTTP 400 `Invalid schema for function '…': "…" is not a "regex"` — failing the whole request, not just that tool. Symptom: every message in a session errors, for every model, while that MCP server is enabled; OpenCode Zen models tolerate it, deepseek does not.
   - Fix: gate the offending tools globally in opencode.jsonc — `"tools": { "sanity_dataset_assets_upload_from_url": false, "sanity_media_assets_upload_from_url": false }` (top-level `tools` map = toolId → boolean; MCP tool ids are `<server>_<tool>`).
   - Diagnose ANY such case: (a) `grep level=ERROR ~/.local/share/opencode/log/opencode.log`; (b) hand-probe the MCP over HTTP (`initialize` → `notifications/initialized` → `tools/list`, unwrap the SSE `data:` line) to dump every inputSchema; (c) replay ALL tools to the provider's own API with curl (`{"tools":[…], "messages":[…], "max_tokens":3}`) and drop the offender it names, looping until HTTP 200. That loop is the ground truth — Python's `re` accepts patterns the provider's validator rejects, so never conclude "no bad regex" from a local scan alone.

**3. `small_model` unset routes titles to an unpayable model.** Default Zen title model (`opencode/gpt-5.4-nano`) logged `AI_APICallError: No payment method` on every run (silent — output still looks fine). Set `"small_model": "deepseek/deepseek-flash"` alongside `model`.

**4. Config files merge, and agent overrides shadow the top-level model.** `~/.config/opencode/opencode.json` and `opencode.jsonc` are BOTH loaded and merged; `agent.<name>.model` (and `agents/*.md` frontmatter) overrides the top-level `"model"`. A top-level default silently loses to a stale per-agent override — check this before concluding a provider "works but isn't used".

**5. Re-verify with a real agent loop, not just a ping.** A passing `Respond with exactly: X` does not prove tool schemas are accepted for a *coding* run — but a schema 400 kills the very first ping too, so: run a plain `opencode run '<task asking it to write and execute a file>'` and confirm the artifact exists and runs.
