...
§
Tue AI Update cron (a658b981983a) PAUSED; resumer (7ca3294036eb) fires Aug 17 15:00 EAT → next run Aug 18 12:00 EAT (Aug 11 week skipped, covered by opencode post). Uses blog-drafting skill.
§
...
§
OpenCode (Aug 2026): 1.18.15 + Zen key. Paid unusable. Docs lie — live-test. Use ~/.hermes/scripts/opencode_coder.py "TASK" --dir <path> (auto-picks live free model). Aug 10: 18 atlas-* skills bridged (opencode_skills_bridge.py), 3 subagents (security-auditor/pentest-recon/code-reviewer, 1.18.x tools:{"*":false} format), fuzz=ask, validator skill_validator.py, SkillSpector ~/.hermes/venvs/skillsec. Models + details: LESSONS.md §03 / opencode skill. DeepSeek plans; OpenCode heavy coding. Verify generated code.
§
