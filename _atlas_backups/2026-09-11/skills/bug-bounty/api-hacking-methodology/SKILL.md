...
- **Self-hosted admin dashboards** — see `references/supabase-studio-self-hosted-auth-bypass.md`: env-flag-gated auth wrappers (`IS_PLATFORM && withAuth`), SQL-proxy privilege checks before RCE claims, JWT-secret→JWKS forging, SSRF fetch-proxy host-regex bypasses, docker-inspect/kong.yml deployment recon
...
- **Check for Swagger redirect hubs**: Probe `/swagger.json` and `/swagger/v1/swagger.json` on every API domain. If they respond with 301/302 instead of 200, follow the redirect — it may lead to a central Swagger documentation hub (e.g., `swagger.maventa.com`) serving docs for multiple APIs. Centralized hubs often expose API catalogs you can't find through individual domain probing.
- **Discovery commands**:
  ```bash
  # Check for swagger redirects
  for url in "https://api1.target.com/swagger.json" "https://api2.target.com/swagger/v1/swagger.json"; do
    code=$(curl -s -o /dev/null -w "%{http_code}" "$url")
    redirect=$(curl -s -o /dev/null -w "%{redirect_url}" "$url" 2>/dev/null)
    echo "$code → $url"
    [ -n "$redirect" ] && echo "  ↳ $redirect"
  done
  ```
...
### 1.2b Detecting the "Generic 404" API Gate Pattern

Some APIs return `404` for BOTH non-existent paths AND existing-but-auth-protected endpoints, deliberately hiding the distinction. The response body often includes a telltale phrase like `"or you don't have access to it"`:

```json
{"code":"not_found","message":"Requested resource not found or you don't have access to it","details":null}
```

**What this tells you:**
- The API server is running and serving requests (not a dead domain)
- ALL paths returning this 404 are potential endpoints behind auth
- You CANNOT distinguish "doesn't exist" from "needs auth" by status code alone — you need a valid token
- The message literal `"...or you don't have access..."` is a deliberate design choice, not standard HTTP

**Detection script:**
```bash
BASE="https://api-stage.target.com"
# If ALL of these return the same 404 with the same body → generic 404 gate
for path in "/api/v1" "/api/v1/users" "/api/v1/admin" "/nonexistent12345"; do
  body=$(curl -s "$BASE$path" 2>/dev/null)
  echo "$path → $(echo "$body" | python3 -c 'import sys,json; d=json.load(sys.stdin); print(d.get("message",""))' 2>/dev/null || echo "$body" | head -c 80)"
done
```

**When you see this pattern:** The API is likely well-secured at the gateway/auth layer. Focus on:
1. Finding a way to get credentials (register on the associated web app, mobile API keys, leaked creds)
2. Testing with a valid auth token to reveal which paths are real
3. Checking for monitoring/health endpoints that may bypass the gate (often `/health`, `/monitoring`, `/version`, `/status` return 200)
...
- **OIDC / IdentityServer fingerprinting** (Duende/IdentityServer4):
  - Signs: URLs with `/connect/authorize`, `/connect/token`, `/signin-oidc`, `response_type=code%20id_token` in the OAuth flow
  - Cookies: `.AspNetCore.Cookies`, `idsrv.session`, `idsrv.xsrf`
  - Headers: Duende IdentityServer adds specific `x-frame-options` and `content-security-policy` headers
  - Direct path behavior: `/Account/Register`, `/connect/login?client_id=...` may return 404 when accessed directly but work through the proper redirect flow (the server validates the client_id and redirect_uri against a known registry)
  - Stage environments often use a redirect chain: `app.stage.target.com` → `identity.stage.target.com` → `connect.identity.stagaws.target.com` (composite identity layer split across cloud providers)
  - **Detection**:
    ```bash
    # Following the redirect chain reveals the architecture
    curl -sI -o /dev/null -w "Final URL: %{redirect_url}" "https://app.stage.target.com"
    # Check for well-known OIDC config
    curl -s "https://identity.stage.target.com/.well-known/openid-configuration" | head -5
    ```
  - Registration availability: If the IdentityServer login page has no "Create account" link and only shows "Email + Next → Password + Sign In", registration likely happens through a different portal (admin console, separate registration site, or is invite-only for stage environments)
  - **Hybrid cloud IDP architecture**: Visma uses AWS (stagaws) for the identity provider itself but Azure (trafficmanager.net) for the app frontends. The OIDC authorize endpoint lives on AWS while the web apps run on Azure. This split means:
    - The auth server (AWS) is a separate codebase from the apps (Azure)
    - Token validation happens cross-cloud — test if the Azure app trusts tokens minted by the AWS IDP
    - Rate limits and WAF rules may differ between the two clouds
  - **OIDC scope enumeration**: Custom scopes like `vsn-assistant-api:chat` reveal undocumented AI backend services. See `references/oidc-scope-api-discovery.md`.
...
### 1.5d2 SPA Backend Discovery via JS Bundle (Azure Static Web Apps / Cloud SPA Pattern)

Modern SPAs on Azure Static Web Apps, Vercel, or Cloudflare Pages hide API endpoints behind a static frontend. The API runs as separate Functions/microservices. The key technique: **find the backend URL embedded in the JS bundle, then probe it directly — it often has different auth, CORS, and rate-limit rules than the frontend.**

**Detection pattern:** The frontend responds with `200` and loads a React/Vue/Angular SPA, but all `/api/*` paths return `404` from the static host (Azure Static Web Apps returns a branded `404: Not found` page). This means the SPA calls an external backend — the URL is baked into the JS bundle.

**Extraction workflow:**
```bash
# 1. Fetch the SPA, extract JS bundle URLs
curl -sk "https://target-spa.com" | grep -oP 'src="[^"]*\.js[^"]*"' | head -5

# 2. Download the main JS bundle and search for cloud backend URLs
curl -sk "https://target-spa.com/static/js/main.abc123.js" \
  | grep -oP 'https?://[a-zA-Z0-9._-]+\.azurewebsites\.net[^\s,;)"'']*' | sort -u

# Also search for other cloud providers
grep -oP 'https?://[a-zA-Z0-9._-]+\.(azure-api|azurewebsites|netlify|vercel|fly\.dev|herokuapp|render|railway)[^\s,;)"'']*'

# 3. Probe the discovered backend directly
curl -sk -o /dev/null -w "%{http_code}" "https://discovered-backend.azurewebsites.net/"
curl -sk -o /dev/null -w "%{http_code}" "https://discovered-backend.azurewebsites.net/api/chat"
curl -sk -o /dev/null -w "%{http_code}" "https://discovered-backend.azurewebsites.net/swagger.json"

# 4. Check CORS headers and security posture of the backend
curl -sk -D - -o /dev/null "https://discovered-backend.azurewebsites.net/api/agents" 2>/dev/null | head -20

# 5. OPTIONS preflight reveals CORS policy
curl -sk -X OPTIONS -D - -o /dev/null \
  -H "Origin: https://target-spa.com" \
  -H "Access-Control-Request-Method: POST" \
  "https://discovered-backend.azurewebsites.net/api/chat"
```

**What backend probing reveals:**
- **405 vs 404**: `405` means the route exists but method not allowed — confirms an active API behind auth
- **"Invalid token format"**: The backend validates JWT format — confirms OAuth2/OIDC auth
- **Rate limit headers** (`x-rate-limit-limit`, `x-rate-limit-remaining`): Reveal throttling posture
- **CSP headers** (`frame-ancestors: 'none'`): Security hardening level
- **SignalR hubs** (`/messageRelayHub`, `/negotiate`): Real-time WebSocket endpoints
- **Swagger/OpenAPI**: Sometimes the backend has swagger exposed even when the frontend doesn't

**Real example — Visma AI Assistant (Jun 2026):**
- Frontend: `aiassistant.stage.vismaonline.com` (Azure Static Web App, React SPA, 200)
- JS bundle at `/static/js/main.aa6127e9.js` contained: `https://vsit-aiassistant-stg.azurewebsites.net/`
- Backend probe: root returned "AI Assistant", `/api/chat` returned 405 (route exists, method blocked), invalid Bearer token returned "Invalid token format.", rate limited at 200 req/min
- Backend had stricter CORS than frontend — locked to the frontend origin
- SignalR hub at `/messageRelayHub` returned 401 (needs auth)

**Pitfall**: The backend may have different auth requirements than the frontend. Test endpoints both unauthenticated AND with tokens obtained through the frontend's auth flow. The 405 response on the frontend vs 404 on the backend tells you the route exists at the Function level but is blocked by the Static Web Apps routing config.
...
### 1.5b OpenSocial Gadget Discovery

Game platforms (Nutaku, Kongregate, Armor Games) use OpenSocial gadgets — XML files at `gadgetUrl` that define game infrastructure. Find them by:

1. Intercept catalog/list API responses and search nested objects for `gadgetUrl` or `socialApi`
2. Gadget XMLs reveal: `consumerKey`/`consumerSecret` (OAuth 1.0a), payment handlers, event endpoints
3. Probe each discovered `gadgetUrl` and its associated endpoints:
   ```bash
   # Gadget XML format:
   <Module>
     <ModulePrefs>
       <Require feature="opensocial-0.8" />
       <Link rel="payment.handler" href="https://game-server.com/payment" />
       <Link rel="event.addapp" href="https://game-server.com/addapp" method="POST" />
     </ModulePrefs>
     <Content type="html">...</Content>
   </Module>
   ```
4. Test game backend endpoints (often unauthenticated): `?do=addapp`, `?do=auth`, `?do=payment`
5. The `do=auth` endpoint may return `tempId` — used for game session authentication
6. Game frames (Unity WebGL) load at `www/index.php?tempId=<tempId>` — the tempId is obtained from the OSAPI viewer profile

**Key checks**: Payment handler endpoints with error stacks (trace IDs, exception messages). Unevaluated event handlers may be exploitable without auth.

### 1.5c Mobile APK Analysis for API Discovery
...
### 2.2b OAuth 1.0a — 2-Legged (Consumer-Only) Exploitation

OpenSocial (OSAPI) and legacy social platforms use OAuth 1.0a with two modes:

**3-legged** (full user delegation): consumerKey + consumerSecret + oauth_token + oauth_token_secret.
Signing key: `consumerSecret + "&" + tokenSecret` (includes token).

**2-legged** (application-only, no user context): consumerKey + consumerSecret ONLY.
Signing key: `consumerSecret + "&"` (trailing ampersand, empty token secret).
User identified via `xoauth_requestor_id` query parameter instead of OAuth token.

Test pattern:
```python
import hmac, hashlib, base64, time, random, string, urllib.parse

def sign_2legged_oauth(method, url, consumer_key, consumer_secret, extra_params=None):
    params = {
        'oauth_consumer_key': consumer_key,
        'oauth_nonce': ''.join(random.choices(string.ascii_letters+string.digits, k=12)),
    ...
    base_str = f"{method.upper()}&{urllib.parse.quote(url.split('?')[0], safe='')}&{urllib.parse.quote(param_str, safe='')}"
    signing_key = f"{consumer_secret}&"  # trailing &, no token secret
    signature = base64.b64encode(hmac.new(signing_key.encode(), base_str.encode(), hashlib.sha1).digest()).decode()
    params['oauth_signature'] = signature
    return params

# Usage:
auth_params = sign_2legged_oauth("GET", url, consumer_key, consumer_secret,
                                  {'xoauth_requestor_id': user_id})
```

**Impact signals from OSAPI endpoints:**
- `GET /people/{userId}/@self` — User profile (nickname, grade, friends)
- `GET /people/{userId}/@friends` — Social graph
- `GET /appdata/{userId}/{groupId}/{appId}` — Game data (high scores, progress)

**Auth pass/fail detection**: 401 = OAuth rejected (wrong signing/creds). 500 = OAuth accepted but user lookup failed (confirms signing is correct — user directory may be empty).

### 2.2c OAuth 2.0 Attacks
...
### 2.2d OIDC Scope Mining — Discovering Hidden APIs from OAuth Scope Lists

**Concept**: The OIDC `.well-known/openid-configuration` endpoint exposes `scopes_supported` — a list of every registered scope for that identity server. Scope names often follow patterns that reveal hidden API endpoints, internal services, and MCP servers. This is passive recon — you don't need a valid token.

**Real-world value**: The scope list tells you what APIs exist BEFORE you have credentials, so you know what to target when you get access.

**Scope pattern analysis:**

| Pattern | What It Reveals | Example |
|---------|----------------|---------|
| `resource:action` | API resource + operation | `vsn-assistant-api:chat`, `ea:sales` |
| `resource:sub:action` | Nested resource | `hubix-stage:customer-invoice:peppol` |
| `product-api:scope` | Product-specific API | `eplus-ai-agent:vectors:admin` |
| `product-mcp:mcp` | MCP server exists | `vsn-assistant-mcp:mcp`, `visma.index.mcp:execute` |
| `*ApiWellKnownEndpoint:update` | Internal/auto-update endpoints | `8377193880ApiWellKnownEndpoint:update` |
| `*:*` with stage names | Staging environments | `hubix-stage:*`, `business-graphql-dev:*` |
| `agent_builder:registry:*` | Agent/plugin registry | `agent_builder.registry:install` |
| `token-exchange` grant type | Privilege escalation vector | Cross-scope token exchange |

**Extraction workflow:**

```bash
# Step 1: Fetch the OIDC config
curl -sk "https://identity.target.com/.well-known/openid-configuration" | python3 -c "
import sys, json
d = json.load(sys.stdin)
print('Scopes:', json.dumps(d.get('scopes_supported', []), indent=2))
print('Grant types:', d.get('grant_types_supported', []))
"

# Step 2: Classify scopes by service
# Look for patterns like:
#   {service-name}-api:{action}     → REST API
#   {service-name}-mcp:{action}     → MCP server
#   {product}:{resource}:{action}   → Product API
#   stage/test/{env}:*              → Staging endpoints
#   numeric*ApiWellKnownEndpoint    → Internal auto-registration

# Step 3: Cross-reference scope names with JS bundle
# When you find e.g. 'vsn-assistant-mcp:mcp', search for 'vsn-assistant' or 'mcp' in the app's JS bundles

# Step 4: Test scope with OAuth token if available
# Request a token with the discovered scope:
curl -sk -X POST "https://identity.target.com/connect/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "grant_type=client_credentials&client_id=...&client_secret=...&scope=vsn-assistant-api:chat"
# unauthorized_client = scope exists but needs user auth
# invalid_scope = scope doesn't exist or has different name
# success = you got a token!

# Step 5: Check for token-exchange grant type (privilege escalation)
# If 'urn:ietf:params:oauth:grant-type:token-exchange' is in grant_types_supported:
# Try exchanging a low-privilege token for a higher-privilege one:
curl -sk -X POST "https://identity.target.com/connect/token" \
  -H "Content-Type: application/x-www-form-urlencoded" \
  -d "grant_type=urn:ietf:params:oauth:grant-type:token-exchange&client_id=...&client_secret=...&subject_token=$LOW_TOKEN&subject_token_type=urn:ietf:params:oauth:token-type:access_token&scope=vsn-assistant-api:inspect"
```

**Real example — Visma Connect (Jun 2026):**
```bash
# From the staging OIDC endpoint at connect.identity.stagaws.visma.com
# scopes_supported revealed:
#   vsn-assistant-api:chat      → AI Assistant chat API (not documented elsewhere)
#   vsn-assistant-api:inspect   → AI Assistant inspect endpoint
#   vsn-assistant-api:internal  → Internal assistant API
#   vsn-assistant-mcp:mcp       → MCP (Model Context Protocol) server
#   visma.net.erp.ai.mcp:execute → AI MCP execution
#   agent_builder.registry:*    → Agent builder (plugin system)
#   SMARTSKILLMCP_DEV:*         → Smart Skills MCP dev instances
#   eplus-ai-agent:vectors:admin → AI agent vector DB admin
#   VFRAiGw:mcp:tools:*         → VFR AI Gateway MCP
#   token-exchange grant type   → Potential privilege escalation
```

**Key insight**: The scope name `vsn-assistant-api:chat` directly told us there's an assistant API with a chat endpoint. We found the backend URL by searching the JS bundle and confirmed the endpoint exists at `vsit-aiassistant-stg.azurewebsites.net`. Without the OIDC scope list, we wouldn't have known what to look for.

### 2.2e OAuth2 Client Credential Error Diagnosis

When probing an OAuth2 identity server's token endpoint, the error code tells you exactly what's happening behind the scenes:

| Error | Meaning | What to Do |
|-------|---------|-----------|
| `invalid_client` | Client ID doesn't exist OR secret is wrong | Check if client ID exists on different environment (dev/stage/prod), try different auth method (basic vs POST) |
| `unauthorized_client` | Client EXISTS and secret is CORRECT, but grant/scope combo not allowed | Client requires a different grant (e.g., authorization_code, not client_credentials) or scope is restricted |
| `invalid_scope` | Client exists but this scope doesn't apply | Try `openid` or empty scope first, check OIDC config for valid scopes |
| `invalid_grant` | Grant format is valid but data is wrong | Wrong password, expired auth code, etc. |

**Critical diagnostic:** `unauthorized_client` vs `invalid_client` is the most important distinction:
- `invalid_client` = dead end (wrong environment or bad creds)
- `unauthorized_client` = CREDENTIALS ARE REAL, just need the right flow

**Auth method matters:** Try BOTH `client_secret_post` (in body) and `client_secret_basic` (Authorization header):
```bash
# client_secret_post
curl -sk -X POST "https://identity/token" -d "grant_type=...&client_id=X&client_secret=Y&scope=..."

# client_secret_basic
B64=$(printf "%s:%s" "$CID" "$CSEC" | base64 -w0)
curl -sk -X POST "https://identity/token" \
...
4. **Two-phase SSRF testing** (critical for webhooks/connectors):
   - **Phase A — creation validation**: The format/URL check when configuring the webhook. Often weaker — may accept redirect chains to internal IPs.
   - **Phase B — execution validation**: The connection-level check when the webhook fires. Stronger — UrlBlocker may catch redirect targets.
   - Test each phase independently: first confirm creation bypass (302 to internal IP accepted), then test execution (does UrlBlocker catch it?).
...
### 6.6 MCP Server Reconnaissance (see references/mcp-server-recon.md)
See `references/mcp-server-recon.md` for full methodology on probing Model Context Protocol (MCP) servers — an emerging AI API attack surface discovered during Visma recon (Jun 2026).

### 6.7 Business Logic Flaws (from Bug Bounty Bootcamp Ch17)
...
- `references/mcp-server-recon.md` — MCP (Model Context Protocol) server reconnaissance: detection, probing, auth testing, and attack vectors for this emerging AI API surface (Visma Spiris MCP, Jun 2026).
- `references/auth0-attack-vectors.md` — Auth0 by Okta recon: /co/authenticate, email template Liquid SSTI, enterprise connection bypass, Management API endpoints.
- `references/nutaku-hal-recon-worked-example.md`
...
- `references/sfcc-commerce-cloud-recon.md` — Salesforce Commerce Cloud (SFCC/Demandware) reconnaissance. OCAPI version probing, client ID discovery, SLAS JWT analysis, WebDAV endpoint mapping, storefront pipeline enumeration, and common auth error patterns. Covers both OCAPI and SCAPI API generations.
- `references/saml-idp-recon.md` — SAML Identity Provider reconnaissance for Shibboleth and SimpleSAMLphp. Metadata extraction, certificate analysis, discovery feed parsing, and common endpoint probing.
- `references/multi-target-recon-workflow.md` — Multi-target parallel recon workflow across Intigriti, Bugcrowd, and other platforms. Program selection heuristics, handling auth walls, and common auth patterns encountered across targets.
