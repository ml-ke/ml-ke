...
### 4. SSRF Testing (Cross-Reference)

SSRF testing methodology — including DNS bypass domains, local PoC confirmation, webhook verification, DNS rebinding TOCTOU, redirect chaining, extended test vector sets, and fix patterns — is consolidated in the **`api-bug-bounty-methodology`** skill under **Step 5: SSRF Testing** and its reference file `references/ssrf-dns-bypass-techniques.md`.

See that skill for:
- DNS resolver bypass domains (localtest.me, lvh.me, *.nip.io, *.sslip.io)
- Full test vector set (direct IPs, DNS bypass, URL parser differentials, credential confusion)
- SSRF confirmation via local server PoC (preferred) and webhook (fallback)
- DNS rebinding TOCTOU detection
- Node.js fix patterns
- Bypass targets by environment (AWS/GCP/Azure metadata endpoints, Docker/K8s, internal services)
- GitLab-specific SSRF vectors including Workhorse sendurl/path mapping analysis
- Redirect chaining and URL parser differential exploitation

For source-code-audit-specific SSRF detection patterns (grep-based static analysis for validation functions), see **section 3** above (grep patterns for `fetch()`, URL validation functions, and third-party URL import features).
...
## Full-Stack Re-Audit Playbook (read-only, verified Aug 2026)

When the target is a user-owned codebase that already has an audit doc (e.g. `docs/security-audit.md`), don't start from zero:

1. **Read the prior audit first.** Structure the session as "verify each prior finding: OPEN / FIXED / PARTIAL with current file:line evidence", then hunt NEW areas the old audit missed. A status column per finding is what the owner actually needs to track progress — don't re-derive old findings from scratch.
2. **Delegate by attack surface in parallel** (3 read-only subagents with terminal+file):
   - *Packaged artifact / mobile* — APK: manifest (exported components, debuggable, cleartext, allowBackup), `strings` scan of libapp.so/kernel_blob for secrets + compiled API base URLs (a release APK hardwired to staging is a common find), cert pinning, root/emulator detection, FLAG_SECURE. Use `uv run --with androguard` when aapt/jadx aren't installed.
   - *Backend API* — routes/services/auth: verify prior findings + hunt IDOR, OTP flows, JWT alg/claims, SQL interpolation, money logic (TOCTOU, amount validation, callback/signature enforcement).
   - *Pipeline/infra* — workflows (secrets handling, branch/dispatch restrictions, SHA pinning, permissions blocks), migrations (destructive statements, embedded secrets), `_headers`/wrangler, ignore files vs secrets, docs-vs-reality claims.
   Require file:line evidence per finding, explicit severity, and "no code changes".
3. **Verify the top finding live, non-destructively, on staging only.** For auth/signature gaps, use a harmless event type + random ID (e.g. forged webhook `notificationType=9999_pentest` with a random `requestId`) — it proves the endpoint accepts unsigned input WITHOUT mutating real state. Never fire real transfers/OTPs; test rate limiting/user enumeration with unknown accounts.
4. **Record authorization + scope** in an engagement folder (target URL(s), authorization basis, out-of-scope production, ≥200ms rate limit) before any active request.
5. **Write the report as a status table** — prior findings status + new findings + live-evidence section + numbered implementation order. Deliver the file; offer to append it to the repo's audit doc rather than editing repo files unprompted ("don't change code" often extends to docs).
...
- **`api-bug-bounty-methodology` SKILL.md → Step 5: SSRF Testing and `references/ssrf-dns-bypass-techniques.md`** — Comprehensive SSRF methodology (bypass domains, local PoC, webhook, DNS rebinding, fix patterns, case studies).
...
- **Direct**: Input containing conflicting instructions (\"Ignore earlier directives...\")
