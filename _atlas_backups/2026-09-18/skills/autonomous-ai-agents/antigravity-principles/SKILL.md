...
## CLI plugin/skill mechanics (verified 2026-09-05, agy 1.1.27)

- **`agy plugin install <local-plugin-dir>`** is the verb for a local plugin bundle; `agy plugin import` is ONLY for migrating Gemini/Claude sources and rejects paths ("could not detect extension type"). Install validates first, then copies into `~/.gemini/antigravity-cli/plugins/<name>/` (source untouched — reinstallable). Registry: `agy plugin list` (JSON imports).
- **Plugin layout**: `plugin.json` + `skills/<skill-name>/SKILL.md` (+ per-skill support dirs). Manifest only needs `{"$schema": "https://antigravity.google/schemas/v1/plugin.json", "name", "description"}` — name regex `^[a-zA-Z0-9-_]+$`; extra keys (author/version) tolerated by validate but schema says additionalProperties false.
- **Skill frontmatter tolerates foreign keys** (Hermes `metadata.hermes.*`, long descriptions) — no normalization needed when porting; `agy plugin validate <dir>` reports "N processed".
- **Global (non-plugin) skills**: any markdown file with name+description frontmatter in `~/.gemini/antigravity-cli/skills/` becomes a slash command; project-local ones live in `.agents/skills/`. Plugin skills surface in-session under `/skills` and as slash commands.
- **EOL builds**: an old `agy` prints "This version ... no longer supported. Run agy update, /logout, then log in again" — `agy update` swaps the binary in place (no re-login needed if the keyring session survives).
- **Hermes skill store hygiene**: `~/.hermes/skills` holds internal dot-dirs (`.hub/quarantine`, `.archive`, `.curator_backups`) — prune `.*` when bulk-porting or you'll ship dead/quarantined skills; true live count excludes them.
- **Verify ports end-to-end**: install, `agy plugin validate` on the installed dir, then `agy -p "name two skills ..."` — an obscure correct answer (e.g. repo-specific skill names) proves in-session loading, not hallucination.
