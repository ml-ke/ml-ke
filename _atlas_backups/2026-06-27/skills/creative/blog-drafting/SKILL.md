...
# 2. Check deployment status via GitHub API (reliable, no CDN dependency)
DEPLOY_ID=$(curl -s "https://api.github.com/repos/ml-ke/ml-ke/deployments?environment=github-pages&per_page=1" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d[0]['id'] if d else '')")
STATUS=$(curl -s "https://api.github.com/repos/ml-ke/ml-ke/deployments/${DEPLOY_ID}/statuses" | python3 -c "import sys,json; d=json.load(sys.stdin); print(d[0]['state'] if d else 'unknown')")
echo "Deployment status: $STATUS"  # "success" means the build finished
...
# 4. Retry the direct permalink (may 404 for first ~30s)
for i in 1 2 3; do
...
**What NOT to do:** Do NOT report a failure to the user or abort the cron if the initial `curl` to the direct permalink returns 404. The push succeeded, the deployment API will show "success" within ~30s, and the homepage will list the post. The direct permalink 404 during the first 30-60s is normal GitHub Pages CDN propagation delay.

**Key timing observations from production runs:**
- Deployment API status → "success": ~34s after push
- Homepage listing updates: ~30-45s after push (same window as deploy success)
- Direct permalink returns 200: ~45-60s after push (CDN propagation tail)
