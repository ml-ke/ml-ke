---
name: opencode
description: "Delegate coding to OpenCode CLI. Includes free-model picker + auto-fallback coder wrapper (Aug 2026)."
version: 1.3.0
author: Hermes Agent
license: MIT
...
### Custom agents (agents/ dir — currently empty on this machine)

Define in `~/.config/opencode/agents/<name>.md` (global) or `.opencode/agents/` (project). The filename becomes the agent name. Frontmatter: `description` (required), `mode: primary|subagent`, `model`, `temperature`, `permission`, `prompt` (or `prompt: {file:...}`).

Good defaults for a security-auditor subagent:
```markdown
---
...
permission:
  ...
  bash: deny
---
You are a security auditor. Load the relevant tob-* skill before reviewing.
Report findings with severity and remediation. Never edit files.
```

### Setup reality check (this machine, Aug 2026)

- 177 skills installed from 9 repos: tob-* (77, Trail of Bits security), flutter (22), expo (21), anthropic (17), dart (12), android (12), vercel (9), google (5), use-railway (1 custom)
- Config comment says 176 — count drifted; skills come from git clones and need periodic refresh
- The Hermes skill library (~/.hermes/skills, 40+ security methodology skills) is NOT visible to OpenCode — a sync/bridge would expose IDOR/JWT/SAML methodology to coding sessions (planned improvement, not yet built)
