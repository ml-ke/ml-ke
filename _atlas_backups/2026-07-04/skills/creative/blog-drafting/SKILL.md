...
for r in json.load(sys.stdin).get('workflow_runs', []):
    print(f'  {r[\"status\"]:10s} conclusion={r.get(\"conclusion\",\"-\"):10s} sha={r[\"head_sha\"][:8]}')
  "
```
...
curl -s "https://api.github.com/repos/ml-ke/ml-ke/actions/runs?per_page=1" | \
  python3 -c "import sys,json; r=json.load(sys.stdin)['workflow_runs'][0]; print('Build:', r['status'], r.get('conclusion','-'), 'sha='+r['head_sha'][:8])"
...
curl -s "https://api.github.com/repos/ml-ke/ml-ke/actions/runs?per_page=3" | \
  python3 -c "import sys,json; [print(f'  {r[\"status\"]} conclusion={r.get(\"conclusion\",\"-\")} sha={r[\"head_sha\"][:8]}') for r in json.load(sys.stdin).get('workflow_runs', [])]"
```
...
import sys, json
for r in json.load(sys.stdin).get('workflow_runs', []):
    print(f'  {r[\"status\"]} conclusion={r.get(\"conclusion\",\"-\")} sha={r[\"head_sha\"][:8]}')
" 2>/dev/null
