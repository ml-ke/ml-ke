...
- **False positives in secret scan**: The scanner may flag its own regex patterns in documentation files. The current grep exclusion list (in the `grep -v` pipeline inside `scan_for_secrets`) covers:
  - `atlas-sync.sh` — the script's own scan patterns match against themselves
  - `_hermes_backups/` — historical backup snapshots
  - `hermes-backup/SKILL.md` — this skill's "Secret Scan Patterns" table contains the actual regex patterns as documentation, which the scanner then matches against
  - Common placeholders (`xxx`, `...`, `YOUR`, `your`, `<your`, `sk-...`, `ghp_...`, `example`, `placeholder`, `test`)
  - Build artifacts (`__pycache__`, `node_modules`)

  When adding new scan patterns, update the exclusion list in `atlas-sync.sh` line ~144 to cover any documentation files that contain the patterns. Use the exact assertion syntax: `\\|hermes-backup/SKILL\\.md`
