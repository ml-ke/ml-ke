...
resolves to the GitHub Pages blog. `ml.co.ke` has been observed returning a
Sedo parked page instead of the Chirpy blog — post verification against a
parked domain gives false results.

```bash
# Quick check: does the domain serve blog content?
curl -s https://ml.co.ke | grep -c "jekyll-theme-chirpy\|Chirpy\|ml-ke\|Model Serving\|posts" || echo "⚠️  No blog content detected — custom domain may be parked/offline"

# Fallback: verify the GitHub Pages deployment directly via Actions API
...
If the custom domain shows a parked page (Sedo, etc.), the DNS for `ml.co.ke`
is not pointing to GitHub Pages. The post IS still published to the repo and
the Actions build completes successfully — the site just isn't reachable at
the custom domain until DNS is fixed.
...
```bash
# Check new posts appear on the site (works only if custom domain resolves to blog)
curl -s https://ml.co.ke | grep -oP "(?<=/posts/)[^\"'/]+" | sort -u 2>/dev/null || echo "Custom domain unreachable"

# Verify via Actions API instead
curl -s "https://api.github.com/repos/ml-ke/ml-ke/actions/runs?per_page=3" | \
  python3 -c "import sys,json; [print(f'  {r[\"status\"]:10s} conclusion={r.get(\"conclusion\") or \"-\":10s} sha={r[\"head_sha\"][:8]}') for r in json.load(sys.stdin).get('workflow_runs', [])]"
```
...
# 2. Check build status via GitHub Actions API (more reliable than deployments API)
...
# 3. Check the homepage (served from previous deploy, always available)
curl -s https://ml.co.ke | grep -oP "(?<=/posts/)[^\"'/]+" | sort -u | grep -i "SLUG"
# If the slug is listed, the new deploy has reached the homepage CDN

# 4. Retry the direct permalink (may 404 for first ~45s)
for i in 1 2 3; do
  CODE=$(curl -s -o /dev/null -w "%{http_code}" "https://ml.co.ke/posts/SLUG/")
  if [ "$CODE" = "200" ]; then
...
**What NOT to do:** Do NOT report a failure to the user or abort the cron if the initial `curl` to the direct permalink returns 404. The push succeeded, the Actions run will show "completed success", and the homepage will list the post. The direct permalink 404 during the first ~45-75s is normal GitHub Pages CDN propagation delay.
