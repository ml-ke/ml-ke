...
### OpenCode + DeepSeek broken — 3 stacked bugs, fixed (2026-09-19)
- Symptom: plain `opencode run` died with `UnknownError … "Unexpected server error" ref=err_xxxx` (the ref appears NOWHERE in the log — don't chase it).
- BUG A: `~/.config/opencode/opencode.json` pinned `"model": "deepseek/deepseek-chat"` — not in OpenCode's deepseek registry (`deepseek-flash | deepseek-v4-flash | deepseek-v4-flash-vision-exp | deepseek-v4-pro`; `opencode models | grep deepseek`). An unknown model id surfaces as that opaque server error. Fix: `deepseek/deepseek-v4-pro`.
- BUG B (15× in the log): Sanity MCP declares `filename.pattern = "^[^/\\\0]+$"` on `dataset_assets_upload_from_url` + `media_assets_upload_from_url`. DeepSeek validates tool schemas with a strict regex parser and 400s the ENTIRE request (`Invalid schema for function '…' : "…" is not a "regex"`), so every deepseek session dies on the first message while sanity MCP is on. Fix: top-level `"tools": {"sanity_dataset_assets_upload_from_url": false, "sanity_media_assets_upload_from_url": false}` in opencode.jsonc — the other 39 sanity tools + context7 + gh_grep validate fine.
- BUG C: `small_model` unset → titles sent to `opencode/gpt-5.4-nano` → "No payment method" error on every run (silent, logged only). Fix: `"small_model": "deepseek/deepseek-flash"`.
- Diagnosis recipe that found all three: `grep level=ERROR ~/.local/share/opencode/log/opencode.log` → hand-probe each MCP (HTTP `initialize` then `tools/list`, unwrap the SSE `data:` line) → replay ALL tool schemas to the provider's own API with curl `tools:[…]` and delete offenders until it returns 200. That loop names the exact function the provider chokes on.
- Config gotcha: `opencode.json` AND `opencode.jsonc` in ~/.config/opencode are both loaded and merged, and per-agent `agent.<name>.model` overrides top-level `model` — a top-level default can be silently shadowed (was shadowed by `agent.build=opencode/mimo-v2.5-free`). Removing the agent overrides made deepseek the real default; `opencode_coder.py --smoke` still returns SMOKE_OK for the free-Zen fallback path.
- Verified: `opencode run` (default) → deepseek-v4-pro, file-write+bash agent loop executed, `--model deepseek/deepseek-flash` OK, Zen wrapper OK, no new ERROR lines after the fix.
...
### Weekly learning sweep (2026-09-22) — collection seams + registry-scale scanner verdicts
- arxiv 2609.13321 **SkillSeam**: audit the COLLECTION, not just the file ("a folder of competent skills is not yet a reliable system"). Six testable seams with measured channels: flattened persistence hierarchy +60% loaded tokens; dangling anchor +64% tokens / −3.1pp accuracy; synonymous alias (duplicate dir/name) noncanonical routes 0/32→15/32; overlapping lanes ownership conflicts 0/16→14/16; bland triggers 3/32→30/32 conflicts +3.7× tokens; granularity mis-mix −12.5pp (largest drop). Applied as `skill-quality-audit` **Step 7** + `references/collection-level-seams.md`.
- arxiv 2609.17274 (61,990 registry skills): the three scanners **disagreed on 23,702**; post-adjudication sensitivity **21.67%-61.06%** → a single scanner's "0 findings" means UNMEASURED, not clean. arxiv 2609.14079 SkillSecurer: LLM-in-the-loop was the only compared scanner at 100% detection (>17% of sampled popular skills had latent injection risk). → weekly pipeline gained a bounded LLM second opinion over only the CHANGED skills.
- arxiv 2609.09233 Subagents vs Agent Skills: for long-horizon work, invoke a heavy skill package as a **subagent (fresh context)** when it exposes a clear input/output contract — loading its instructions into the main context degrades as the horizon grows; cost is coordination tokens. Justifies the `delegate_task` + `output_schema` pattern.
- Real defect the new checks caught: OpenCode loads **seven** skill roots and dedupes by `name` with `~/.config/opencode/skills` winning over `~/.agents/skills` — 58 names were in both, and one shadowed copy was **newer and provably wrong** (it reinstated `api.choicebankapi.com` as the live ChoiceBank base URL; DNS-verified 2026-09-22: NXDOMAIN, while `baas.choicedigitalbank.com` resolves with a valid cert). Backup `~/.agents/skills-backup-2026-09-22/` + shadow removed; detector runs weekly.
- New pipeline steps 4-5 (both scripted, both exit 0 so they can never block): `skill_root_seam_check.py` (shadowed/divergent copies across roots) and `skill_second_opinion.py` (cisco-ai-defense/skill-scanner on PyPI + DeepSeek over changed skills). **`gh skill`** (gh ≥ 2.98, preview) is the cheapest official skill discovery surface — but its default install root `~/.agents/skills` may never be loaded by the host agent; install with `--agent`.
...
### Mistral Vibe CLI (mistral-vibe) config (2026-09-19)
- Config: ~/.vibe/config.toml (provider/model/active_model, bash allowlist). Secrets: ~/.vibe/.env (GLOBAL_ENV_FILE, loaded at startup; chmod 600). Shell env wins over .env.
- Defaults are Mistral-native: provider `mistral` -> https://api.mistral.ai/v1, api_key_env_var MISTRAL_API_KEY, default model alias `mistral-medium-3.5` (name mistral-vibe-cli-latest). Cheaper: mistral-vibe-cli-fast, ministral-8b-latest.
- A custom provider (e.g. OpenRouter free model) in config.toml REPLACES the default routing; remove the [[providers]]/[[models]] blocks + pin active_model to fall back to Mistral.
- Working MISTRAL_API_KEY reused from ~/ProG/RICH/.env (32 chars); free/api plan returned 402 Payment Required in Sep 2026, works now.
- Verify a switch by tailing ~/.vibe/logs/vibe.log for `Model call completed model=...` and the newest ~/.vibe/logs/session/*/meta.json (`"provider": "mistral"`, api.mistral.ai).
