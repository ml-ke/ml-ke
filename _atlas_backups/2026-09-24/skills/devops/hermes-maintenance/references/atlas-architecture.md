...
## Verifying a sync run (don't false-alarm)

The script's own output is a self-report. Confirm with three cheap checks:

1. `cd ~/ProG/ATLAS && git rev-parse HEAD origin/main` — the two SHAs must match. Same for the ml-ke mirror.
2. `ls ~/ProG/ATLAS/_secrets/ | grep $(date +%F)` — today's `env-*.gpg`, `config-*.gpg`, `auth-*.gpg` (GPG, local only, never pushed).
3. `ls ~/ProG/ATLAS/_hermes_backups/$(date +%F)/` — MANIFEST.md + config/ jobs/ memory/ scripts/ skills/.

Expected, NOT a failure: the script prints "Skills backed up: 204" while the backup tree holds ~192 `SKILL.md`, because rsync skips `.archive/` dirs. The delta equals the archived-skill count (`comm -23` of live vs backup paths proves it). Never re-run the script just to "fix" a count mismatch — a re-run only produces an empty commit.
