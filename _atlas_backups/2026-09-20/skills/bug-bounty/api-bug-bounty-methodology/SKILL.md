...
description: Systematic approach to finding vulnerabilities in web APIs for bug bounty programs. Covers recon, IDOR, mass assignment, SSRF, auth bypass, and business logic testing. Uses Notion API as primary practice target. Includes absorbed program-specific intelligence for Discourse, Elastic Kibana, Supabase, and Salesforce Commerce Cloud (SFCC).
---
...
### DNS Rebinding TOCTOU (Separate-Check-Then-Fetch Gap)

A subtle SSRF bypass exists when DNS resolution and HTTP fetch happen in separate, uncoupled steps. The `dns.lookup()` resolves and checks the IP against a blocklist, but `fetch()` makes a **fresh** DNS resolution. An attacker can exploit this gap:
...
grep -rn "dns.lookup\|dns.resolve" --include='*.ts' --include='*.js' . \
  | grep -v node_modules | grep -v '.test.'
# Then check if fetch() or http.request() uses a SEPARATE resolution from the check
```

**Fix**: Use `fetch()` with a URL that has the pre-resolved IP (not the hostname), or pass the `agent` option with the pre-resolved IP. Alternatively, use `dns.resolve4()` and construct the URL from the resolved address.

### Extended Test Vector Set

When testing URL validation functions, use the full suite of test vectors to cover all bypass categories:
...
# === DNS RESOLVER BYPASS (validation gap) ===
http://localtest.me/       http://lvh.me/
http://127.0.0.1.nip.io/   http://169.254.169.254.nip.io/latest/meta-data/
http://10.0.0.1.nip.io/    http://192.168.1.1.nip.io/
http://127.0.0.1.sslip.io/ http://169.254.169.254.sslip.io/

# === URL PARSER DIFFERENTIALS (Node.js normalizes these) ===
http://0x7f000001/         # hex → 127.0.0.1
http://2130706433/          # decimal → 127.0.0.1
http://0/                   # zero shorthand → 0.0.0.0
http://[::ffff:7f00:1]/    # IPv4-mapped IPv6 → 127.0.0.1
http://[::1]/               # IPv6 loopback

# === CREDENTIAL CONFUSION ===
http://user:pass@127.0.0.1/          # credentials parsed, hostname = 127.0.0.1
http://127.0.0.1:80@evil.com/        # parser differential: which host?
```

See `references/ssrf-dns-bypass-techniques.md` for the complete catalog including cloud metadata endpoints (AWS/GCP/Azure), Docker/K8s internal services, IPv6 variants, CGNAT bypass, and redirect-based bypass.

### Node.js Fix Pattern

When the validation does not resolve DNS before making the request, the fix is to add DNS resolution with IP blocklist checking:
...
For web/edge runtimes without raw DNS (Cloudflare Workers, Deno Deploy), block known DNS resolver services by pattern:
```typescript
...
> **Reference**: `references/ssrf-dns-bypass-techniques.md` — comprehensive catalog of all DNS bypass domains, IP obfuscation formats (decimal/hex/octal/IPv6-mapped), URL parser differential exploitation, redirect-based bypass, cloud metadata endpoints per provider (AWS/GCP/Azure), DNS rebinding TOCTOU details, and a Python test script.
...
> **Cross-reference**: For systematic **white-box** auditing of a single codebase (grep-based static analysis, compiled/C++ library audit, MPC crypto audit, AI platform/SDK audit), load the `source-code-security-audit` skill. This skill covers black-box live-API testing; that one covers source-level review. The two are complementary — use source-code-security-audit when you have the repo, this skill when you only have endpoints.
...
## Appendix A: Program-Specific Intelligence

This section provides condensed methodology for each bug bounty program that was consolidated into this umbrella skill. Each subsection summarizes the program, attack surface, key findings, and points to a detailed reference file.

### A.1 Discourse (HackerOne)

**Target**: Discourse open-source forum software | **Bounties**: Severity at discretion | **Test instance**: try.discourse.org (resets daily) | **Stack**: Ruby on Rails + Ember.js + PostgreSQL

**Attack Surface Highlights:**
- **SSRF**: Discourse's FinalDestination library has comprehensive protection (32+ private IP ranges including CGNAT, DNS resolution before connect, thread-safe). **Not attackable** via basic SSRF.
- **AI Plugin Audit Log IDOR**: `AiApiAuditLog.find(params[:id])` — unscoped integer ID lookup. **Column constraint**: `t.string` → `varchar(255)` — only first 255 chars of prompts stored.
- **52 plugins**: newer ones (AI, Chat, Subscriptions) may have weaker auth than core.
- Known CVEs: 246 total, avg CVSS 4.0. Common themes: routing misconfigs, data exposure, plugin-specific bugs.

**Reference**: `references/discourse-recon-summary.md` — full SSRF protection analysis, codebase map, CVE study, untested attack vectors.

### A.2 Elastic Kibana (HackerOne)

**Target**: Elastic Kibana ($150–$7k bounties) | **Source**: ~/Dev/elastic/kibana/ | **Stack**: TypeScript/Node.js (110K files) | **Test**: cloud.elastic.co or self-hosted trial

**Attack Surface Highlights:**
- **Connector SSRF (CONFIRMED)**: `allowedHosts` defaults to `['*']` on cloud. Not even basic IP filtering.
  - `.webhook` connector accepts arbitrary internal IPs
  - `.http` (Workflows) connector has `params.url` override at execution time
  - Internal scan: 10.x.x.x subnet routable but port-filtered; GCP metadata blocked by hypervisor
  - Reported as **Elastic/001** (CVSS 8.6 High)
- **Dual-allowlist risk**: Actions plugin (`xpack.actions.allowedHosts`) and Workflows (`workflowsExecutionEngine.http.allowedHosts`) are separate — configuring one but not the other leaves a gap.
- **Vega Visualization XSS**: `ast: true` sandbox in vega-interpreter (2.2.1). CSP on cloud: `script-src 'self'` + no `unsafe-eval`. Same-origin SSRF via Vega data source (`../api/status`) works regardless.
- **Console/Dev Tools proxy**: Host-validated against configured ES hosts — well-defended.
- **Saved Objects import**: Clean NDJSON pipeline, no injection.

**References:**
- `references/elastic-kibana-cloud-live-testing-results.md` — deployment info, confirmed SSRF, connector types, reports filed
- `references/elastic-kibana-kibana-codebase-map.md` — v9+ directory layout by area (CSP, connectors, console proxy, saved objects)
- `references/elastic-kibana-connector-ssrf-testing.md` — API-driven connector SSRF automation
- `references/elastic-kibana-cloud-csp-analysis.md` — CSP on cloud vs default source config
- `references/elastic-kibana-workflows-dual-allowlist-risk.md` — two separate `allowedHosts` configs analysis
- `references/elastic-kibana-vega-visualization-testing.md` — Vega XSS/CSS injection tests

### A.3 Supabase (HackerOne)

**Target**: Supabase open-source Firebase alternative | **Status**: VDP (recognition-only; paid bounties planned) | **Scope**: 18 assets | **Stack**: TypeScript/Node.js monorepo, Postgres

**Attack Surface Highlights:**
- **Self-hosted auth bypass (CONFIRMED)**: `apiWrapper()` checks `IS_PLATFORM && withAuth` → on self-hosted (`IS_PLATFORM=false`), auth is **never** checked even for endpoints with `{ withAuth: true }`.
- **Bare handler pattern**: Some endpoints (`/api/v1/projects/[ref]/api-keys`, `/api/edge-functions/test`) have **no apiWrapper at all** → no auth check on any deployment.
- **Full attack chain** (6 steps, all auth-free):
  1. Create admin user (POST /api/platform/auth/default/users)
  2. Extract SUPABASE_SERVICE_KEY (bare handler /api-keys)
  3. Extract AUTH_JWT_SECRET (settings endpoint)
  4. Arbitrary SQL via pg-meta auth bypass → RCE via COPY TO PROGRAM
  5. Internal SSRF via edge-functions/test
  6. Full container compromise
- **MCP Server audit**: No SSRF at runtime (URLs hardcoded or via typed openapi-fetch). Prompt injection protections via `<untrusted-data>` wrappers.

**References:**
- `references/supabase-studio-api-audit.md` — endpoint-by-endpoint analysis of Studio API with auth patterns
- `references/supabase-escalation-patterns.md` — post-exploitation chains (service key → SQL → RCE → SSRF)
- `references/supabase-mcp-audit.md` — MCP server code-level audit (tool analysis, prompt injection, SSRF vectors)

### A.4 Salesforce Commerce Cloud / SFCC (Intigriti)

**Target**: Salesforce Commerce Cloud (formerly Demandware) — cloud-based e-commerce platform used by European retailers (Torfs, Coolblue, etc.) | **Test instance**: www.torfs.be (SFCC v25.5) | **Payout**: Tier 2 (€100–€6,500) | **Stack**: OCAPI / SCAPI / SLAS JWT auth, WebDAV for file management, Cloudflare CDN

**Fingerprinting:** SFCC instances are identifiable by cookies (`dwsid` Base64 session ID, `cc-at_{Site-ID}` SLAS JWT, `dwanonymous_{hash}`), headers (`x-dw-request-base-id`), and paths (`/on/demandware.store/`, `/on/demandware.servlet/webdav/`, `/dw/shop/{version}/`).

**API Surfaces:**
- **OCAPI** (Open Commerce API): Older API at `/dw/shop/{version}/{resource}?client_id={id}`. Version probing: 404 = not enabled, 400 with `MissingClientIdException` = version EXISTS. Client IDs are often the Site-ID (e.g., `Torfs-Webshop-BE`). Resources: baskets, customers/self, orders, product_search, categories, promotions, sessions, gift-certificates.
- **SCAPI/SLAS** (Shopper Commerce API + Login Service): Newer API via Salesforce CDN. Guest/user JWTs in `cc-at_{Site-ID}` cookie (ES256-signed). The JWT `isb` claim decodes user info bundle (email, display name, global customer ID). The `scp` claim lists OAuth scopes (baskets, orders, payments, promotions, gift-certs, addresses, etc.)
- **WebDAV**: `/on/demandware.servlet/webdav/Sites/{Cartridges|Impex|Code|Static}` — 401 (BASIC auth) = exists but needs credentials. Code paths explicitly blocked (403).
- **Storefront pipelines**: `/on/demandware.store/Sites-{Site-ID}/{locale}/{Pipeline-Name}` — server-side rendering endpoints for Login, Register, Cart, Product, Search, Account, ConsentTracking, etc.

**Common Attack Vectors:**
- IDOR on orders/baskets (baskets are session-associated, not user-associated)
- Promotions / voucher abuse (stackable or reusable promo codes, enumerable codes)
- Gift certificate manipulation (balance manipulation, cross-user application)
- Payment instrument access (read/add/remove saved payment methods on another account)
- Pricing manipulation (server-side validation gaps on bundles/tiered pricing)
- Cross-tenant / multi-site (vulnerability on one domain often works on all instances)
- Address IDOR using user-chosen address names (`homeaddress`, `Neighbor`, `billing`, `shipping`)
- CSRF token discovery and reuse on state-changing pipeline calls
- SLAS JWT replay / session bridging across domains
- Pipeline enumeration via HTML data attributes (`data-action="Address-List|Order-History"`)

**Reference**: `references/torfs-recon-session.md` — full recon session on Torfs (Intigriti), including OCAPI probing results, SLAS JWT decode, SCAPI scopes, WebDAV responses, CSRF tokens, pipeline discovery, and session auth state extraction.
