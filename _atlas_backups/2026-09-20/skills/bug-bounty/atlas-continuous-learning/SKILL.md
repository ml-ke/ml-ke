...
## Delegation Budget Gate (added Sep 15 2026 — cloudflare/security-audit-skill + Hermes delegation docs)

Subagent waves are where budget quietly dies, and a wave that returns nothing is worse
than no wave. Reserve BEFORE launching; if the reserve cannot be funded, launch nothing.

1. **Reserve first.** Count: hunters + one post-wave coverage critic + one *distinct*
   final-clean critic + ~1 verifier per expected candidate (≥30% of the remaining balance
   when unsure). Only the remainder goes to hunters.
2. **Gate on fundability.** If the minimum reserve does not fit the budget, launch ZERO
   agents and report `incomplete: budget_cannot_fund_recon_and_reserves`. The Aug 2
   Etsy/Skoda wave that burned 600s on unscoped recon is this failure without a number.
3. **Use `output_schema`, not prose summaries.** `delegate_task` accepts a JSON Schema per
   task; the child is told to return only matching JSON, and the parent gets
   `schema_valid` + `schema_errors` plus ONE bounded correction turn. Prose summaries force
   manual re-verification (Aug 2 lesson: "subagent summaries are self-reports"); a
   schema-valid record goes straight into the findings ledger.
4. **Verifiers must try to REFUTE.** Give each candidate to a fresh child that did not hunt
   it, tasked with disproving it. Verdicts are `confirmed` / `needs_validation` /
   `rejected` — never a narrative.
5. **Children do not own processes.** A child's background processes die at delegation
   teardown; instruct it to wait for its own builds/tests before returning.

Contract, schema, and the exact `delegate_task` call shape:
`references/delegation-budget-and-verdicts.md`.
