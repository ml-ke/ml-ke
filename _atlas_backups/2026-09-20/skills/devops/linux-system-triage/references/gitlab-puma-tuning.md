...
# Count workers from the HOST (containers share the host PID namespace):
ps aux | grep 'puma.*cluster worker' | wc -l   # expect ~2-3 (master + workers)
```

`gitlab-ctl reconfigure` output ends with "gitlab Reconfigured!" when successful. Puma master PID stays; only the worker count changes. GitLab is installed only in Docker on this box (no /etc/gitlab on host, no gitlab-ctl binary on host).
