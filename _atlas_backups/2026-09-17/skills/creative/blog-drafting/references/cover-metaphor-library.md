...
- The catalog is NOT exhaustive: a few covers predate it and are referenced but unnumbered (e.g. the "two ledgers" reconciliation cover `cover-reconciliation-analytics-fintech.svg`, Aug 15 post, cited in #31). Always `ls assets/blog/cover-*.svg` before designing so you don't collide with an uncatalogued cover — entry #33 below was designed against a live listing.
...
## 31. Payment Hub Spray — Gateway + Diverging Outflow + Detection Threshold
**File:** `cover-flutterwave-fraud-anatomy.svg`
**SVG Elements:**
- Central payment gateway hub: 100×100 rounded rect labeled "PAYMENT HUB" (monospace, cyan #00d2ff), with a soft radial glow circle behind it
- Diverging attack flows: 9 straight red lines from hub edge outward, each with marker-end arrowheads — 4 bright (#ff6b6b), 5 dim (opacity 0.45)
- Flagged/frozen accounts: 4 "ACCT" blocks ABOVE the threshold line (red stroke), 2 of them with small green "FROZEN" rounded badges
- Evaded deposits: 5 "ACCT" blocks BELOW the threshold line (dim red stroke) — the "kept under the radar" cluster
- Detection threshold: dashed yellow (#ffd93d) horizontal line with monospace labels "DETECTION THRESHOLD" and "deposits kept below trigger limits"
- Standard shell: background grid (opacity 0.03), title at y=62, subtitle at y=96, "ml-ke.github.io" bottom-right
- Color: cyan (#00d2ff) hub, red (#ff6b6b) attack flows, yellow (#ffd93d) threshold, green (#6bcf7f) frozen/recovered
**Use for:** payment-infrastructure breach anatomy, mule-network laundering, threshold-evasion fraud (distinct from the ledger/reconciliation cover — that one compares two ledgers; this one shows one hub spraying to many accounts).

---

## 32. KYC/AML Automation Playbook — Checkpoint Conveyor
**File:** `cover-kyc-aml-automation-playbook.svg`
**SVG Elements:**
- Conveyor belt: horizontal rounded rect (1080×26, rx=13, dark slate `#1e293b`→`#243447` gradient) with tick marks, spanning the frame
- 5 checkpoint gates as arch shapes (rounded-top paths) standing over the belt, each holding a person glyph (circle head + shoulder arc):
  1. DOC (cyan #00d2ff) — document check
  2. LIVENESS (green #6bcf7f)
  3. WATCHLIST (red #ff6b6b, gate fill red at 0.08 opacity, red flag pennant above gate) — the flagging gate
  4. MONITOR (purple #a78bfa)
  5. RECON (yellow #ffd93d)
- Documents travelling the belt: 4 small rounded rects (56×40) with inner bar lines — 3 passed with green check paths (cyan/green/purple strokes), 1 **FLAGGED** (red stroke, rotated −8°, red glow filter, two red ✗ strokes) parked at the WATCHLIST gate
- "FLAGGED" red label below the flagged doc; tagline under belt: "Every checkpoint independent · none trusts the one before it"
- Filters: `redGlow` (feGaussianBlur stdDeviation 8) + `softGlow` (4) — reuse pattern for alert emphasis
- Standard shell: grid opacity 0.03, title y=62, subtitle y=102, "ml-ke.github.io" bottom-right
- Color: cyan/green/purple/yellow passing gates, red (#ff6b6b) for the flagged document
**Use for:** layered-controls playbooks (KYC/AML automation, multi-gate defense), threshold-bypass prevention. Distinct from the payment-hub spray (#31) which shows the *attack*; this one shows the *defense pipeline* catching an item mid-flow.

---

## 33. Anomaly Detection for Reconciliation — Radar/Sonar Lock-on

**File:** `cover-anomaly-detection-reconciliation.svg` (Aug 31, 2026 — verified, ffmpeg → WebP 25 KB)
**SVG Elements:**
- Radar origin at bottom-left (`translate(150, 520)`): cyan radial glow disc (0.18→0), 3 range-ring arcs (`M 0 0 L dx dy A r r 0 0 0 dx2 dy2 Z` paths, opacity 0.15–0.25), translucent sweep wedge (`M 0 0 L 1080 -150 L 1080 -10 Z`, cyan fill 0.05), bright leading sweep line with arrow marker, dish (Q-curve + stem + hub circle), "SCAN" monospace label
- Normal transaction stream: ~28 small green (#6bcf7f) dots jittered in a band rising slightly left→right (y≈452→396), thin cyan axis line, "NORMAL TRANSACTION STREAM" label
- Distributed-attack cluster: 5 dim red dots (opacity 0.45) sitting *inside* the band with a yellow "BELOW THRESHOLD" label — conveys the Flutterwave lesson without drawing a threshold line (keeps it distinct from #31)
- The anomaly: red glow-filtered dot at (866,222) with ring, 34px crosshair reticle (4 ticks + corner brackets), dashed lock-on line from radar origin to the anomaly, red monospace "ANOMALY" label
- Standard shell: grid opacity 0.03, title y=62, subtitle y=100, "ml-ke.github.io" bottom-right
- Color: cyan (#00d2ff) radar/stream, green (#6bcf7f) normal dots, red (#ff6b6b) anomaly, yellow (#ffd93d) sub-threshold
**Use for:** statistical/ML anomaly detection posts, threshold-evasion fraud, "catching what fixed thresholds miss" angles. Distinct from #30 (monitoring dashboard panels), #31 (payment-hub spray), and the uncatalogued "two ledgers" reconciliation cover.
**Technique note:** forward references to `<marker>`/`<defs>` defined later in the file resolve fine in ffmpeg/cairosvg — no need to reorder.

## Technique notes (verified Sep 3, 2026 — population-drift covers)

- **Accurate distribution curves beat hand-written Beziers.** For bell curves
  / gaussian shapes, generate the SVG from a Python helper that samples the
  math function and emits path `L` segments (e.g. `gauss_path(mu, sigma, amp,
  base_y)` → `M x0,y0 L x1,y1 ...`). 6px sampling at 1200×630 is smooth.
  Hand-beziering a normal curve is fiddly and easy to get wrong.
- **Check label-vs-line collisions from coordinates BEFORE rendering.** A
  `text-anchor="middle"` label centered on the same x as a vertical needle or
  axis line overlaps it (Sep 3: "PSI 1.22" label sat directly on its own
  needle line at x=820). Anchor labels left/right (`text-anchor="end"` at
  `NX-14`) of vertical elements, or place them above curve tops in clear
  space. Compute curve y at label x (`y = base - amp*exp(-(x-mu)²/2σ²)`) to
  confirm the label clears the curve.
- **Iterate via the generator script, not by hand-editing the emitted SVG.**
  Keep the Python generator in /tmp (or tools/), regenerate + reconvert after
  each geometry fix — hand-patching coordinates in the SVG desyncs the
  generator and makes the next fix harder.
- **Literal `<` or `>` in text** (zone labels like "OK <0.10") must be
  XML-entity-encoded as `&lt;` / `&gt;`; ffmpeg/cairosvg render them fine.
  The naive amp-escape grep (`&[^a]`) false-flags these valid entities — use
  the PCRE-lookahead check in SKILL.md Pitfall #9.
